...
% set the fading flag to 1 for simulating fading event
b_error_conseq = [0 1 2 3 4];

% iterate across different b_error_conseq_symbols
for index_b_error_conseq=1:length(b_error_conseq)
...
%                 symbol_sequence_mapped_uncoded = mapping(x_info_uncoded,mod_scheme(index));
                ...
                %% Fading (for coded bits)

                % choose randomly where the fading will start
                start_of_fading = randi([1 N-max(b_error_conseq)],1,1);
                
                % initialize the symbol sequence with the ones before
                % fading
                symbol_sequence_coded_after_fading = symbol_sequence_mapped_coded;
                
                % apply fading
                symbol_sequence_coded_after_fading(1:size(symbol_sequence_coded_after_fading,1),start_of_fading:start_of_fading + b_error_conseq(index_b_error_conseq))= 0*size(symbol_sequence_coded_after_fading,1);
                ...
                if strcmp(mod_scheme(index),'BPSK')
                    for index5 =1:N
                        sum = sum + symbol_sequence_coded_after_fading(index5)^2;   
                    end
                else
                    for index5 =1:N
                        sum = sum + symbol_sequence_coded_after_fading(1,index5)^2 + symbol_sequence_coded_after_fading(2,index5)^2;
                    end
                end
                E_symb_avg = sum/N;
                ...
                % 4. generate noise
                noise = sqrt(sigma_square/2)*randn(size(symbol_sequence_coded_after_fading));

                % 5. adding noise
                symbol_sequence_noisy_coded = symbol_sequence_coded_after_fading + noise;
...
%                 BER_per_mod_scheme_uncoded(index_symbol,index_Eb_No) = errors_in_current_block_symbol_uncoded /length(x_info_uncoded);            
        ...
        BER_coded_all_schemes{index_b_error_conseq,index} = BER_per_mod_scheme_coded;
...
Avg_BER_coded = cell(length(b_error_conseq),length(mod_scheme));

for index_b_error_conseq=1:length(b_error_conseq)
    for index=1:length(mod_scheme)
        Avg_BER_coded{index_b_error_conseq,index} = mean(BER_coded_all_schemes{index_b_error_conseq,index},1);
    end
end
...
%% Plots
figure()
semilogy(Eb_No_dB, Avg_BER_coded{1,1}, 'b--*', 'LineWidth', 2, 'MarkerSize',10)
hold on;
semilogy(Eb_No_dB, Avg_BER_coded{2,1}, 'r--+', 'LineWidth', 2, 'MarkerSize',10);
hold on;
semilogy(Eb_No_dB, Avg_BER_coded{3,1}, 'g--o', 'LineWidth', 2, 'MarkerSize',10);
hold on;
semilogy(Eb_No_dB, Avg_BER_coded{4,1}, 'k--d', 'LineWidth', 2, 'MarkerSize',10);
hold on;
semilogy(Eb_No_dB, Avg_BER_coded{5,1}, 'm--s', 'LineWidth', 2, 'MarkerSize',10);
hold on;
title('Interleaving against fading: 16-QAM, m=4, 16X16 interleaver');
xlabel('Eb/No in dB');
...
grid on;
set(gca,'FontSize',18);
leg=legend({'b-err = 1','b-err = 2','b-err = 3','b-err = 4','b-err = 5'},'Location','southwest');
set(leg,'FontSize',18);
hold off;
